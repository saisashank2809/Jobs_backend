...
class SupabaseAdapter(DatabasePort):
    ...
    async def create_job(self, data: dict[str, Any]) -> dict[str, Any]:
        def _create():
            return self._client.table("jobs_jobs").insert(data).execute()
    ...
    async def update_job(self, job_id: str, data: dict[str, Any]) -> None:
        await run_in_threadpool(
            lambda: self._client.table("jobs_jobs").update(data).eq("id", job_id).execute()
        )
    ...
    async def find_job_by_external_id(
        ...
        result = await run_in_threadpool(_find)
        return result.data if result else None
    ...
    async def delete_job(self, job_id: str) -> None:
        await run_in_threadpool(
            lambda: self._client.table("jobs_jobs").delete().eq("id", job_id).execute()
        )
    ...
    async def update_chat_session(
        ...
        def _update():
            return self._client.table("chat_sessions_jobs").update(data).eq("id", session_id).execute()
            
        try:
            result = await run_in_threadpool(_update)
            if not result.data:
                import logging
                logging.getLogger(__name__).warning("Supabase UPDATE for session %s returned NO data.", session_id)
        except Exception as e:
            import logging
            logging.getLogger(__name__).error("Supabase UPDATE failed for session %s: %s", session_id, e)
            raise
    ...
    async def create_mock_interview(self, data: dict[str, Any]) -> dict[str, Any]:
        def _create():
            return self._client.table("mock_interviews_jobs").insert(data).execute()
        result = await run_in_threadpool(_create)
        return result.data[0]
    ...
    async def update_mock_interview(self, interview_id: str, data: dict[str, Any]) -> None:
        await run_in_threadpool(
            lambda: self._client.table("mock_interviews_jobs").update(data).eq("id", interview_id).execute()
        )
    ...
    async def insert_scraping_log(self, data: dict[str, Any]) -> dict[str, Any]:
        def _insert():
            return self._client.table("scraping_logs_jobs").insert(data).execute()
    ...
    async def update_scraping_log(
        ...
        await run_in_threadpool(
            lambda: self._client.table("scraping_logs_jobs").update(data).eq("id", log_id).execute()
        )
    ...
    async def create_blog_post(self, data: dict[str, Any]) -> dict[str, Any]:
        def _create():
            return self._client.table("blog_posts_jobs").insert(data).execute()
        result = await run_in_threadpool(_create)
        return result.data[0]
